...
#if defined(NDEBUG)
# error "Liteblocks cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb027bcacd1dad153e07f7ab75363456efe05646068f97ac299d13da45b9fbbe4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Liteblocks: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Liteblocks Signed Message:\n";
...
{
    // Liteblocks: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Liteblocks: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Liteblocks: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1528372530, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "25";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fd1a7624b5dedb343f671d9c6232a241743ecccb9faa4a8ba67abf3fb42c6995607b19fcda7ee6fa74010e61c40faf9db939efca9a49e384a10da8f558151e0e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528372530;
        ...
        assert(block.hashMerkleRoot == uint256("0xdaed730a75578782bf7aff8225c4d47e8afbd6866ae0389264cf58dce52ba260"));
...
unsigned char pchMessageStart[4] = { 0x6d, 0xb3, 0xb4, 0x8e }; // Liteblocks: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LiteblocksMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LiteblocksMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LiteblocksMiner(CWallet *pwallet)
{
    printf("LiteblocksMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("liteblocks-miner");
    ...
    try { loop {
        ...
        printf("Running LiteblocksMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LiteblocksMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LiteblocksMiner, pwallet));
}
